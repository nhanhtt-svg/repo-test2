...
import pytest
from math_operations import calculate_discount, is_prime, find_max, process_data
...
class TestIsPrime:
    ...
    def test_prime_numbers(self):
        """Các số nguyên tố"""
        assert is_prime(2) == True   # Sẽ FAIL (hàm kiểm tra range(2, 1) rỗng)
        assert is_prime(3) == True   # Sẽ FAIL
    ...
    def test_non_prime_numbers(self):
        ...
        assert is_prime(4) == False   # Sẽ FAIL (4 % 2 == 0 nhưng range(2, 2) rỗng)
        assert is_prime(9) == False   # Sẽ FAIL
        assert is_prime(15) == False  # Sẽ FAIL
...
def test_prime_edge_cases():
    ...
    assert is_prime(2) == True  # Sẽ FAIL

    # Số 1 không phải nguyên tố
    assert is_prime(1) == False

    # Số lớn không nguyên tố
    assert is_prime(49) == False  # 49 = 7*7, Sẽ FAIL
